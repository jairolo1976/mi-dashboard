...
export default function AlumnoCard({ alumno, onUpdated, onDeleted }) {
  ...
  const save = async e => {
    ...
    try {
      await fetch(`http://localhost:3001/alumnos/${alumno.id}`, {
        method: 'PUT',
  ...
  const del = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/alumnos/${alumno.id}`, {
        method: 'DELETE'
      });
      ...
      {editing ? (
        <form onSubmit={save} className="space-y-2">
          ...
          <div className="flex space-x-2">
            ...
            <button type="button" onClick={() => setEditing(false)} className="bg-gray-400 text-white px-3 py-1 rounded hover:bg-gray-500">
              Cancelar
            </button>
          </div>
        </form>
        ...
        <>
          ...
          <p><span className="font-medium">Grupo sanguíneo:</span> {alumno.grupoSanguineo}</p>
          {alumno.alergias && <p><span className="font-medium">Alergias:</span> {alumno.alergias}</p>}
          {alumno.medicacion && <p><span className="font-medium">Medicación:</span> {alumno.medicacion}</p>}
          <div className="mt-4 flex space-x-2">
            <button onClick={() => setEditing(true)} className="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600">
              Editar
            </button>
            <button onClick={del} className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">
              Eliminar
            </button>
          </div>
        </>
